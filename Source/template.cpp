...
vec3 operator / (const float& s, const vec3& v) { float vs = 1.0f / s; return vec3(v.x * vs, v.y * vs, v.z * vs); }
vec3 operator / (const vec3& v, const float& s) { float vs = 1.0f / s; return vec3(v.x * vs, v.y * vs, v.z * vs); }
...
#endif
	Sprite icon{ new Surface("assets/logo.jpg"), 1 };
	SDL_Surface* iconSurface;
	iconSurface = SDL_CreateRGBSurfaceFrom(icon.GetBuffer(), 256, 256, 32, 1024, 0xff0000, 0x00ff00, 0x0000ff, 0);
