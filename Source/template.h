...
constexpr int ScreenHeight = 660;
// #define FULLSCREEN
...
static const char* TemplateVersion = "'Sweet Little Lines' Framework | Stefan Pijnacker - Y1 Block B";
...
inline vec3 RandomInUnitSphere()
{
	...
	float lat = acosf(2.0f * u - 1) - (PI / 2.0f);
	float phi = 2.0f * PI * u2;
	float x = cosf(lat) * cosf(phi);
	float y = cosf(lat) * sinf(phi);
	float z = sinf(lat);
